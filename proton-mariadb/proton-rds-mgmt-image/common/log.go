...
// Logger Proton RDS Mgmt服务日志服务，可适配其他日志组件
type Logger interface {
	...
	SetLogLevel(logLevelStr string)
}
...
func initLogger() {
	...
	logHandle.logger.SetFormatter(&logrus.TextFormatter{
		DisableColors:   true,
		ForceQuote:      true,
		TimestampFormat: "2006-01-02T15:04:05.000Z07:00",
	})

	logDir := "/var/log/proton-rds-mgmt/"
	logName := "proton-rds-mgmt.log"
	err := os.MkdirAll(logDir, 0750)
	if err != nil {
		fmt.Println("mkdir err", err)
	}
	logFileName := path.Join(logDir, logName)
	logFile, err := os.OpenFile(logFileName, os.O_RDWR|os.O_CREATE|os.O_APPEND, os.ModeSetuid)
	if err != nil {
	...
	logHandle.logger.SetOutput(fileAndStdoutWriter)

	l = logHandle
}

// SetLogLevel 设置日志级别
func (l *serverLog) SetLogLevel(logLevelStr string) {

	logLevel, err := logrus.ParseLevel(logLevelStr)
	if err != nil {
		l.Errorln("Parse Log Level failed:" + logLevelStr)
		return
	}
