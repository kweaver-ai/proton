...
func newHandler(metrics collector.Metrics, scrapers []collector.Scraper) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		registry.MustRegister(collector.New(ctx, dsn, httpDSN, metrics, filteredScrapers))
...
`)
	...
	//os.Setenv("DATA_SOURCE_NAME", "exporter:mKRb1DAwdWBCBuGs@(10.4.14.11:3306)/")
	//os.Setenv("HTTP_DATA_SOURCE_NAME", "http://127.0.0.1:8888")

	// 从hostname获取idx, exporter-idx 监控db-idx
	hostname := strings.Split(os.Getenv("POD_NAME"), "-")
	host := os.Getenv("DB_POD_NAME_PRE") + "-" + hostname[len(hostname)-1] + "." + os.Getenv("DB_SVC")
	...
	httpDSN = os.Getenv("HTTP_DATA_SOURCE_NAME")
	if len(httpDSN) == 0 {
		httpDSN = "http://127.0.0.1:8888"
	}
