...
func New(ctx context.Context, dsn string, httpDSN string, metrics Metrics, scrapers []Scraper) *Exporter {
...
func (e *Exporter) Describe(ch chan<- *prometheus.Desc) {
	...
	ch <- e.metrics.TableOK.Desc()
}

// Collect implements prometheus.Collector.
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	e.scrape(e.ctx, ch)
	e.scrapeTableStatus(e.ctx, ch)
	ch <- e.metrics.TotalScrapes
	ch <- e.metrics.Error
	e.metrics.ScrapeErrors.Collect(ch)
	ch <- e.metrics.MySQLUp
	ch <- e.metrics.TableOK
}

func (e *Exporter) scrapeTableStatus(ctx context.Context, ch chan<- prometheus.Metric) {
	finshChan := make(chan int)
	tableOK := 1
	// getCheckTableResultUrl := e.httpDSN + "/api/proton-rds-mgmt/v2/corrupt_table_list"
	go func() {
		defer func() { finshChan <- 1 }()
		// resp, err := http.Get(getCheckTableResultUrl)
		// if err != nil {
		// 	log.Errorln("Error scrapeCorruptTables:", err)
		// 	return
		// }
		// var b []string
		// data, err := ioutil.ReadAll(resp.Body)
		// if err != nil {
		// 	log.Errorln("Error read body when scrapeCorruptTables:", err)
		// 	return
		// }
		// err = jsoniter.Unmarshal(data, &b)
		// if err != nil {
		// 	log.Errorln("Error unmarshal body when scrapeCorruptTables:", err)
		// 	return
		// }
		// if len(b) != 0 {
		// 	tableOK = 0
	...
	select {
	case <-ctx.Done():
		e.metrics.TableOK.Set(1)
	case <-finshChan:
		e.metrics.TableOK.Set(float64(tableOK))
	}
}
...
func NewMetrics() Metrics {
	...
	return Metrics{
		...
		MySQLUp: prometheus.NewGauge(prometheus.GaugeOpts{
			...
			ConstLabels: source_label,
		}),
		TableOK: prometheus.NewGauge(prometheus.GaugeOpts{
			Namespace:   namespace,
			Name:        "table_ok",
			Help:        "Whether check mysql table is ok.if check table ok return 1, else return 0",
			ConstLabels: source_label,
		}),
	}
}
